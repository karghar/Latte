...
instance Print TopDef where
  prt i e = case e of
    FnDef funcdef -> prPrec i 0 (concatD [prt 0 funcdef])
    ClassDef id cdef -> prPrec i 0 (concatD [doc (showString "class"), prt 0 id, prt 0 cdef])
    ClassDefExt id1 id2 cdef -> prPrec i 0 (concatD [doc (showString "class"), prt 0 id1, doc (showString "extends"), prt 0 id2, prt 0 cdef])
...
instance Print CDef where
  prt i e = case e of
    ClassBlock classelems -> prPrec i 0 (concatD [doc (showString "{"), prt 0 classelems, doc (showString "}")])

instance Print ClassElem where
  prt i e = case e of
    ClassMeth funcdef -> prPrec i 0 (concatD [prt 0 funcdef])
    ClassAtr type_ id -> prPrec i 0 (concatD [prt 0 type_, prt 0 id, doc (showString ";")])
  prtList _ [] = (concatD [])
  prtList _ (x:xs) = (concatD [prt 0 x, prt 0 xs])
...
instance Print Expr where
  prt i e = case e of
    ...
    EAttrAccess attraccess -> prPrec i 6 (concatD [prt 0 attraccess])
    EMthCall methodcall -> prPrec i 6 (concatD [prt 0 methodcall])
    EArrAccess arrelemaccess -> prPrec i 6 (concatD [prt 0 arrelemaccess])
    ...
    EApp functioncall -> prPrec i 6 (concatD [prt 0 functioncall])
...
instance Print FunctionCall where
  prt i e = case e of
    FunctionCall id exprs -> prPrec i 0 (concatD [prt 0 id, doc (showString "("), prt 0 exprs, doc (showString ")")])

instance Print ArrElemAccess where
  prt i e = case e of
    ArrayElem lvalue expr -> prPrec i 0 (concatD [prt 0 lvalue, doc (showString "["), prt 0 expr, doc (showString "]")])

instance Print AttrAccess where
  prt i e = case e of
    AttrAcc lvalue id -> prPrec i 0 (concatD [prt 0 lvalue, doc (showString "."), prt 0 id])

instance Print MethodCall where
  prt i e = case e of
    MCall lvalue functioncall -> prPrec i 0 (concatD [prt 0 lvalue, doc (showString "."), prt 0 functioncall])

instance Print LValue where
  prt i e = case e of
    LVFunCall functioncall -> prPrec i 0 (concatD [prt 0 functioncall])
    LVMethodCall methodcall -> prPrec i 0 (concatD [prt 0 methodcall])
    LVJustIdent id -> prPrec i 0 (concatD [prt 0 id])
    LVArrayAcc arrelemaccess -> prPrec i 0 (concatD [prt 0 arrelemaccess])
    LVAttrAcc attraccess -> prPrec i 0 (concatD [prt 0 attraccess])
